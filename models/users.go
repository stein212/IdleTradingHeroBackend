...
var UserWhere = struct {
	...
	TelegramID: whereHelpernull_Int{field: "\"users\".\"telegram_id\""},
	CreatedOn:  whereHelpertime_Time{field: "\"users\".\"created_on\""},
}
...
var (
	userAllColumns            = []string{"id", "username", "password", "email", "first_name", "last_name", "telegram_id", "created_on"}
	userColumnsWithoutDefault = []string{"id", "username", "password", "email", "first_name", "last_name", "telegram_id", "created_on"}
...
// MacdStrategies retrieves all the macd_strategy's MacdStrategies with an executor.
func (o *User) MacdStrategies(mods ...qm.QueryMod) macdStrategyQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"macd_strategies\".\"user_id\"=?", o.ID),
	)

	query := MacdStrategies(queryMods...)
	queries.SetFrom(query.Query, "\"macd_strategies\"")

	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"macd_strategies\".*"})
	}
...
func (userL) LoadMacdStrategies(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(qm.From(`macd_strategies`), qm.WhereIn(`macd_strategies.user_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load macd_strategies")
	}

	var resultSlice []*MacdStrategy
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice macd_strategies")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on macd_strategies")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for macd_strategies")
	}
	...
	if singular {
		object.R.MacdStrategies = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &macdStrategyR{}
			}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.UserID {
				local.R.MacdStrategies = append(local.R.MacdStrategies, foreign)
				if foreign.R == nil {
					foreign.R = &macdStrategyR{}
				}
...
func (o *User) AddMacdStrategies(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*MacdStrategy) error {
	var err error
	...
	if o.R == nil {
		o.R = &userR{
			MacdStrategies: related,
		}
	} else {
		o.R.MacdStrategies = append(o.R.MacdStrategies, related...)
	}
